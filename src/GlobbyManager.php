 ...
 * negation patterns, directory expansion, and gitignore integration. This
 * is the main entry point for all glob operations, handling pattern parsing,
 * file system traversal, and result filtering.
 *
 * The manager supports both synchronous and streaming glob operations, with
 * extensive configuration options for filtering, expansion, and gitignore
 * integration.
 *
...
{
    /**
     * Pattern matcher for glob pattern operations including wildcards and recursion.
     */
    private PatternMatcher $matcher;

    /**
     * Gitignore parser for handling .gitignore files and ignore rules.
     */
     ...
     *
     * @param FileSystemAdapter $fs Custom file system adapter for testing or alternative implementations.
     *                              Defaults to native PHP file system operations.
     */
     ...
     *
     * Accepts single or multiple glob patterns with support for negation (patterns
     * starting with !), wildcards (*, ?), character classes ([abc]), and recursive
     * matching (**). Returns sorted array of matching paths or GlobEntry objects.
     *
     * @param  array<string>|string      $patterns Single pattern string or array of patterns to match.
     *                                             Supports negation with ! prefix and all standard glob syntax.
     * @param  null|array<string, mixed> $options  Configuration options for glob behavior including filtering,
     *                                             expansion, and output format. See GlobbyOptions for details.
     * @return array<GlobEntry|string>   Sorted array of matching file paths (strings) or GlobEntry objects
     *                                   when objectMode is enabled. Results are deduplicated by default.
     */
     ...
     *
     * Internal method that performs the actual glob operation with a configured
     * GlobbyOptions instance. Handles pattern separation, directory expansion,
     * negative patterns, gitignore filtering, and result transformation.
     *
     * @param array<string>|string $patterns Single pattern string or array of patterns to match
     * @param GlobbyOptions        $options  Configured options object controlling all aspects of glob behavior
     *
     * @throws BrokenSymbolicLinkException When throwErrorOnBrokenSymbolicLink is enabled and a broken symlink is encountered
     * @throws DirectoryNotFoundException  When the configured cwd doesn't exist
     *
     * @return array<GlobEntry|string> Sorted, filtered array of matching file paths or GlobEntry objects
     */
     ...
     *
     * Memory-efficient alternative to glob() that yields results one at a time
     * instead of building the entire result set in memory. Useful for processing
     * large numbers of files or when early termination is desired.
     *
     * @param  array<string>|string        $patterns Single pattern string or array of patterns to match
     * @param  null|array<string, mixed>   $options  Configuration options for glob behavior. See GlobbyOptions.
     * @return Generator<GlobEntry|string> Generator yielding matching file paths or GlobEntry objects one at a time
     */
     ...
     *
     * Useful for determining if a path is a literal file path or contains
     * wildcards that require glob matching. Detects *, ?, [, ], {, and }.
     *
     * @param  string $pattern The pattern to check for dynamic characters
     * @return bool   True if the pattern contains any glob special characters
     */
     ...
     *
     * Escapes glob metacharacters (*, ?, [, ], {, }, etc.) so that the path
     * will be treated as a literal string in glob operations. Useful when you
     * have a file path that might contain characters with special meaning.
     *
     * @param  string $path The file path to escape and convert to a literal pattern
     * @return string The escaped pattern safe for use in glob operations
     */
     ...
     *
     * Checks if the given path would be ignored based on .gitignore files found
     * in the current directory and parent directories up to the git repository root.
     * Respects negation patterns and directory-specific rules.
     *
     * @param  string                    $path    The file or directory path to check against gitignore rules
     * @param  null|array<string, mixed> $options Configuration options including cwd for resolving relative paths
     * @return bool                      True if the path should be ignored according to gitignore rules
     */
     ...
     *
     * Similar to isGitIgnored but allows specifying custom ignore file patterns
     * (e.g., .dockerignore, .eslintignore). Supports glob patterns for finding
     * multiple ignore files across the directory tree.
     *
     * @param  string                    $path        The file or directory path to check
     * @param  array<string>|string      $ignoreFiles Single pattern or array of patterns to locate ignore files
     * @param  null|array<string, mixed> $options     Configuration options including cwd
     * @return bool                      True if the path should be ignored according to the specified ignore files
     */
     ...
     *
     * Converts input patterns into a structured format compatible with external
     * glob libraries or task runners. Handles pattern expansion, separation of
     * positive/negative patterns, and options normalization.
     *
     * @param  array<string>|string                                                 $patterns Single pattern or array of patterns to process
     * @param  null|array<string, mixed>                                            $options  Configuration options for task generation
     * @return array<array{patterns: array<string>, options: array<string, mixed>}> Array of task objects containing patterns and merged options
     */
