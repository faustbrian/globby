 ...
 *
 * Handles parsing of gitignore pattern syntax and checking if paths match
 * any of the ignore rules. Supports negation patterns (! prefix), directory-only
 * patterns (/ suffix), and glob patterns. Implements gitignore semantics including
 * searching up to repository root and respecting pattern hierarchy.
 *
 * Results are cached per file for performance when checking multiple paths.
 *
...
{
    /**
     * Cache of parsed ignore rules by file path.
     *
     * Stores parsed rules to avoid re-parsing the same ignore files repeatedly.
     * Key is the absolute path to the ignore file, value is array of parsed rules.
     *
     ...
     *
     * @param FileSystemAdapter $fs File system adapter for reading ignore files and checking paths
     */
     ...
     *
     * Searches for .gitignore files from cwd up to git repository root and in
     * subdirectories. Checks if the path matches any ignore patterns, respecting
     * negation rules and pattern order (later rules override earlier ones).
     *
     ...
     * @param  GlobbyOptions $options Configuration options including traversal depth
     ...
     *
     * Similar to isIgnored() but uses custom ignore file patterns instead of .gitignore.
     * Useful for checking against .dockerignore, .eslintignore, or other ignore file formats.
     * Supports glob patterns for finding multiple ignore files.
     *
     * @param  string        $path        The file or directory path to check
     * @param  array<string> $ignoreFiles Patterns or paths to locate ignore files (supports globs)
     * @param  string        $cwd         Current working directory for resolving relative paths
